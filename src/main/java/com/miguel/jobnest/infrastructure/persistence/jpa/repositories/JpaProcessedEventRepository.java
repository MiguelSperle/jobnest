...
public interface JpaProcessedEventRepository extends JpaRepository<JpaProcessedEventEntity, String> {
    @Query(nativeQuery = true, value = "SELECT EXISTS (SELECT 1 FROM processed_events WHERE event_id = :eventId AND listener = :listener)")
    boolean existsByEventIdAndListener(@Param("eventId") String eventId, @Param("listener") String listener);
}
